...
export default function NewAccessPointPage() {
  ...
  const [selectedType, setSelectedType] = useState<'event' | 'documentation' | 'both'>('event');
  const [requireId, setRequireId] = useState(false);
  const [name, setName] = useState('');

  const handleSave = () => {
    if (!name.trim()) {
      toast.error('Please enter an access point name');
      return;
    }
      ...
      <div className="flex-1 ml-[var(--spacing-sidebar)]">
        ...
        <main className="container mx-auto px-4 py-6 max-w-5xl">
          ...
          <div className="flex items-center gap-2 text-xs text-gray-500 mb-6">
            ...
            <span>›</span>
            <span className="font-semibold text-gray-900">Create Access Point</span>
          </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                <div className="space-y-1.5">
                  ...
                  <select className="w-full h-10 px-3 border border-gray-200 rounded-lg text-sm text-gray-700 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20">
